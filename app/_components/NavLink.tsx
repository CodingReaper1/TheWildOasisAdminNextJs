...
function NavLink({ children, href, text }: NavLinkProps) {
  ...
  return (
    <Link
      ...
      className={`inline-flex items-center justify-center gap-0 space-x-0 px-[1.2rem] py-[1.2rem] text-[2.4rem] transition-all duration-300 ${isActive ? "rounded-md bg-gray-50 text-indigo-600 dark:bg-gray-900 dark:text-indigo-600" : "text-gray-400 hover:rounded-md hover:bg-gray-50 hover:text-indigo-600 dark:text-gray-500 hover:dark:bg-gray-900 hover:dark:text-indigo-600"}`}
      href={href}
    >
      {children}
      <MotionComponent
